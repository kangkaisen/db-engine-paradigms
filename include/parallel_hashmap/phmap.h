...
protected:
    ...
    std::tuple<size_t, bool, V> find_or_prepare_update(const K& key, const V& v, size_t hash) {
        auto seq = probe(hash);
        while (true) {
            Group g{ctrl_ + seq.offset()};
            for (int i : g.Match(H2(hash))) {
                if (PHMAP_PREDICT_TRUE(PolicyTraits::apply(
                    EqualElement<K>{key, eq_ref()},
                    PolicyTraits::element(slots_ + seq.offset(i))))) {
                    return std::make_tuple(seq.offset(i), false, PolicyTraits::element(slots_ + seq.offset(i)).second);
                }
            }
            if (PHMAP_PREDICT_TRUE(g.MatchEmpty())) break;
            seq.next();
        }
        return std::make_tuple(prepare_insert(hash), true, 0);
    }

    // template <class K>
    // std::tuple<size_t, bool, double> find_or_prepare_update(const K& key, size_t hash) {
    //     auto seq = probe(hash);
    //     while (true) {
    //         Group g{ctrl_ + seq.offset()};
    //         for (int i : g.Match(H2(hash))) {
...
public:
    ...
    template <class K = key_type>
    iterator find_no_lock(const key_arg<K>& key, size_t hashval) {
        Inner& inner = sets_[subidx(hashval)];
        auto&  set   = inner.set_;
        auto  it = set.find(key, hashval);
        return make_iterator(&inner, it);
    ...
    template <class K = key_type>
    iterator find_no_lock(const key_arg<K>& key) {
        return find_no_lock(key, HashElement{hash_ref()}(key));
    }
...
protected:
    ...
    std::tuple<Inner*, size_t, bool, V>
    find_or_prepare_update(const K& key, const V& v, typename Lockable::UniqueLock &mutexlock) {
        auto hashval = HashElement{hash_ref()}(key);
        Inner& inner = sets_[subidx(hashval)];
        auto&  set   = inner.set_;
        mutexlock    = std::move(typename Lockable::UniqueLock(inner));
        auto  p   = set.find_or_prepare_update(key, v, hashval); // std::pair<size_t, bool>
        return std::make_tuple(&inner, std::get<0>(p), std::get<1>(p), std::get<2>(p));
    }
...
public:
    ...
    template <class K = key_type, class V = mapped_type, K* = nullptr,
        V* = nullptr>
    bool insert_or_update(key_arg<K>&& k, V&& v) {
        return insert_or_update_impl(std::forward<K>(k), std::forward<V>(v));
    }

    template <class K = key_type, class V = mapped_type, K* = nullptr>
    bool insert_or_update(key_arg<K>&& k, const V& v) {
        return insert_or_update_impl(std::forward<K>(k), v);
    }

    template <class K = key_type, class V = mapped_type, V* = nullptr>
    bool insert_or_update(const key_arg<K>& k, V&& v) {
        return insert_or_update_impl(k, std::forward<V>(v));
    }

    template <class K = key_type, class V = mapped_type>
    bool insert_or_update(const key_arg<K>& k, const V& v) {
        return insert_or_update_impl(k, v);
    }
    ...
    template <class K = key_type, class P = Policy>
    MappedReference<P> at_no_lock(const key_arg<K>& key) {
        auto it = this->find_no_lock(key);
        if (it == this->end())
            phmap::base_internal::ThrowStdOutOfRange("phmap at(): lookup non-existent key");
        return Policy::value(&*it);
    }
...
private:
    ...
    template <class K, class V>
    bool insert_or_update_impl(K&& k, V&& v) {
        typename Lockable::UniqueLock m;
        auto res = this->find_or_prepare_update(k, v, m);
        typename Base::Inner *inner = std::get<0>(res);
        if (std::get<2>(res))
            inner->set_.emplace_at(std::get<1>(res), std::forward<K>(k), std::forward<V>(v));
        else
            Policy::value(&*inner->set_.iterator_at(std::get<1>(res))) = std::forward<V>(v) + std::get<3>(res);
