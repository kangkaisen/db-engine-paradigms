...
NOVECTORIZE std::unique_ptr<runtime::Query> q23_hyper(Database& db,
                                                      size_t nrThreads) {
   // --- aggregates
   auto resources = initQuery(nrThreads);

   // --- constants
   auto relevant_brand = types::Char<9>::castString("MFGR#2221");
   auto relevant_region = types::Char<12>::castString("EUROPE");
...
std::unique_ptr<Q23Builder::Q23> Q23Builder::getQuery() {
   using namespace vectorwise;
   auto result = Result();
   previous = result.resultWriter.shared.result->participate();
   auto r = make_unique<Q23>();
...
std::unique_ptr<runtime::Query> q23_vectorwise(Database& db, size_t nrThreads,
                                               size_t vectorSize) {
   using namespace vectorwise;
   ...
   workers.run([&]() {
      Q23Builder builder(db, shared, vectorSize);
   ...
   //       auto orderdate = reinterpret_cast<types::Integer*>(block.data(orderdateAttr));
   //       auto sum = reinterpret_cast<types::Numeric<18, 2>*>(block.data(sumAttr));
   //       for (size_t i = 0; i < elementsInBlock; ++i) {
   //          std::cout <<  shopmode[i] <<  " | " << orderdate[i] << " | " << sum[i] << std::endl;
   //       }
   //    };
   //    std::cout <<  "result count is " << count << std::endl;
   // };
