...
NOVECTORIZE std::unique_ptr<runtime::Query> q22_hyper(Database& db,
                                                      size_t nrThreads) {
   // --- aggregates
   auto resources = initQuery(nrThreads);

   // --- constants
   auto brand_min = types::Char<9>::castString("MFGR#2221");
   auto brand_max = types::Char<9>::castString("MFGR#2228");
   ...
   auto found3 = PARALLEL_SELECT(p.nrTuples, entries3, {
      ...
      if (brand1 <= brand_max && brand1 >= brand_min) {
...
std::unique_ptr<runtime::Query> q22_vectorwise(Database& db, size_t nrThreads,
   ...
   workers.run([&]() {
      Q22Builder builder(db, shared, vectorSize);
